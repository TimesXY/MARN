...
def roc_model(model, data_test):
    ...
    for i, (test_images, test_labels) in enumerate(data_test):
        ...
        # 获取输出
        model_predict = model(test_images)
...
def metrics_model(model, data_test):
    ...
    # 计算召回率
    recall_micro = metrics.recall_score(label_list, score_list, average="macro")
    print('Recall is :', np.around(recall_micro, 4))

    # 计算 F1 值
    f1score_micro = metrics.f1_score(label_list, score_list, average="macro")
    print('F1score is :', np.around(f1score_micro, 4))

    # 计算精准率
    precision_micro = metrics.precision_score(label_list, score_list, average="macro")
